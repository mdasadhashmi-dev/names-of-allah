// AlAkhirStoryView.swift - Generated
import SwiftUI
import AVFoundation
struct AlAkhirStoryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "One day, the sun\nwill be extinguished.", icon: "sun.max.fill") }
                    else if currentScreen == 1 { StoryTextBox(text: "The stars will collapse.\nThe galaxies will vanish.", icon: "sparkles") }
                    else if currentScreen == 2 { StoryTextBox(text: "The earth will be destroyed.", icon: "globe") }
                    else if currentScreen == 3 { StoryTextBox(text: "Every soul will perish.", icon: "figure.fall") }
                    else if currentScreen == 4 { StoryTextBox(text: "And when everything is gone—\n\nAl-Akhir remains.", icon: "infinity.circle") }
                    else if currentScreen == 5 { StoryTextBox(text: "The Last.\n\nWith no end.", icon: "seal.fill") }
                    else if currentScreen == 6 { StoryTextBox(text: "Your life will end.\nThis world will end.", icon: "xmark.circle") }
                    else if currentScreen == 7 { StoryTextBox(text: "But Al-Akhir\nhas no end.", icon: "circle") }
                    else if currentScreen == 8 { StoryTextBox(text: "He will outlast everything—\n\nbecause He is eternal.", icon: "crown.fill") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showTranslation { Text("\"He is the First\nand the Last (Al-Akhir),\nthe Manifest and the Hidden\"").font(.system(size: 20, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Al-Akhir?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "infinity", text: "Al-Akhir has no end—He is eternal.")
                            ExplanationPoint(icon: "xmark.circle", text: "Everything will perish except Al-Akhir.")
                            ExplanationPoint(icon: "seal.fill", text: "After the universe ends, Al-Akhir remains.")
                            ExplanationPoint(icon: "heart.circle", text: "He is your final destination. Prepare to meet Al-Akhir.")
                            Text("He is Al-Akhir—\nthe Last,\nwith no end.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
    ...
    private func autoPlayAyahAudio() {
        if let audioPath = Bundle.main.path(forResource: "al-hadid-57-3-akhir", ofType: "mp3") {
