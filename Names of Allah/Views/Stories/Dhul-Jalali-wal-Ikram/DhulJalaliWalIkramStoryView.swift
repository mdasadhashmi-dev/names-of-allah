// DhulJalaliWalIkramStoryView.swift - Dhul-Jalali wal-Ikram (Possessor of Majesty and Honor)
import SwiftUI
import AVFoundation
struct DhulJalaliWalIkramStoryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "Majestic.\nGlorious.\nHonorable.", icon: "crown.fill") }
                    else if currentScreen == 1 { StoryTextBox(text: "These words describe\nkings and rulers.", icon: "building.2") }
                    else if currentScreen == 2 { StoryTextBox(text: "But true majesty\nbelongs to One—", icon: "sparkles") }
                    else if currentScreen == 3 { StoryTextBox(text: "Dhul-Jalali wal-Ikram—\n\nPossessor of Majesty\nand Honor.", icon: "star.fill") }
                    else if currentScreen == 4 { StoryTextBox(text: "His majesty\nis absolute.", icon: "infinity.circle") }
                    else if currentScreen == 5 { StoryTextBox(text: "His honor\nis eternal.", icon: "checkmark.seal") }
                    else if currentScreen == 6 { StoryTextBox(text: "Every moment of majesty\nyou witness—\n\nis a reflection of His.", icon: "arrow.down") }
                    else if currentScreen == 7 { StoryTextBox(text: "Honor Him—\n\nfor He is the source\nof all honor.", icon: "hands.sparkles") }
                    else if currentScreen == 8 { StoryTextBox(text: "Dhul-Jalali wal-Ikram—\n\nMajesty and Honor\npersonified.", icon: "heart.circle.fill") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showArabic { VStack(spacing: 16) { Text("وَيَبْقَىٰ وَجْهُ رَبِّكَ ذُو ٱلْجَلَـٰلِ وَٱلْإِكْرَامِ").font(.system(size: 26, weight: .bold, design: .serif)).foregroundColor(.white).multilineTextAlignment(.center).lineSpacing(10).padding(.horizontal, 30); Text("Surah Ar-Rahman 55:27").font(.system(size: 14, weight: .light)).foregroundColor(Color.appGold.opacity(0.7)) }.transition(.opacity.combined(with: .move(edge: .top))) }
                    if showTranslation { Text("\"And there will remain the Face of your Lord,\nOwner of Majesty (Dhul-Jalali) and Honor (wal-Ikram)\"").font(.system(size: 19, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Dhul-Jalali wal-Ikram?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "crown", text: "Dhul-Jalali wal-Ikram possesses absolute majesty and honor.")
                            ExplanationPoint(icon: "infinity", text: "His majesty is eternal—all worldly majesty fades.")
                            ExplanationPoint(icon: "sparkles", text: "Every glimpse of majesty is a reflection of His.")
                            ExplanationPoint(icon: "hands.sparkles", text: "Honor Dhul-Jalali wal-Ikram—the source of all honor.")
                            Text("Dhul-Jalali wal-Ikram—\nMajesty and Honor\neternal.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
    ...
    private func autoPlayAyahAudio() {
        if let audioPath = Bundle.main.path(forResource: "ar-rahman-55-27-dhul", ofType: "mp3") {
