// AshShaheedStoryView.swift - Ash-Shaheed (The Witness)
import SwiftUI
import AVFoundation
struct AshShaheedStoryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "You thought\nno one saw.", icon: "eye.slash") }
                    else if currentScreen == 1 { StoryTextBox(text: "You hid it well.", icon: "lock.fill") }
                    else if currentScreen == 2 { StoryTextBox(text: "You deleted the messages.\nYou cleared the history.", icon: "trash.fill") }
                    else if currentScreen == 3 { StoryTextBox(text: "But Ash-Shaheed—\nThe Witness—\n\nsaw everything.", icon: "eye.fill") }
                    else if currentScreen == 4 { StoryTextBox(text: "Every glance.", icon: "eyes") }
                    else if currentScreen == 5 { StoryTextBox(text: "Every thought.", icon: "brain.head.profile") }
                    else if currentScreen == 6 { StoryTextBox(text: "Every intention.", icon: "heart.text.square") }
                    else if currentScreen == 7 { StoryTextBox(text: "Nothing escapes\nAsh-Shaheed.", icon: "infinity.circle") }
                    else if currentScreen == 8 { StoryTextBox(text: "He is witnessing\nthis moment—\n\nright now.", icon: "sparkles") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showArabic { VStack(spacing: 16) { Text("وَٱللَّهُ عَلَىٰ كُلِّ شَىْءٍۢ شَهِيدٌۭ").font(.system(size: 36, weight: .bold, design: .serif)).foregroundColor(.white).multilineTextAlignment(.center).padding(.horizontal, 30); Text("Surah An-Nisa 4:33").font(.system(size: 14, weight: .light)).foregroundColor(Color.appGold.opacity(0.7)) }.transition(.opacity.combined(with: .move(edge: .top))) }
                    if showTranslation { Text("\"And Allah is Witness (Shaheed)\nover all things\"").font(.system(size: 24, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Ash-Shaheed?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "eye.fill", text: "Ash-Shaheed witnesses everything—every thought, glance, and deed.")
                            ExplanationPoint(icon: "lock", text: "Nothing is hidden from Him—He sees what you hide.")
                            ExplanationPoint(icon: "scale.3d", text: "He will testify on Judgment Day about everything you did.")
                            ExplanationPoint(icon: "hands.sparkles", text: "Live knowing Ash-Shaheed is witnessing you right now.")
    ...
    private func autoPlayAyahAudio() {
        if let audioPath = Bundle.main.path(forResource: "an-nisa-4-33", ofType: "mp3") {
