// AlBaqiStoryView.swift - Al-Baqi (The Everlasting)
import SwiftUI
import AVFoundation
struct AlBaqiStoryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "Your house\nwill crumble.", icon: "house.fill") }
                    else if currentScreen == 1 { StoryTextBox(text: "Your wealth\nwill be spent.", icon: "banknote") }
                    else if currentScreen == 2 { StoryTextBox(text: "Your loved ones\nwill die.", icon: "heart.slash") }
                    else if currentScreen == 3 { StoryTextBox(text: "Even YOU\nwill perish.", icon: "figure.stand") }
                    else if currentScreen == 4 { StoryTextBox(text: "Everything\nwill end.", icon: "smoke.fill") }
                    else if currentScreen == 5 { StoryTextBox(text: "Except one—\n\nAl-Baqi.", icon: "infinity.circle") }
                    else if currentScreen == 6 { StoryTextBox(text: "The Everlasting\nwho never ends.", icon: "sparkles") }
                    else if currentScreen == 7 { StoryTextBox(text: "So attach your heart\nto Al-Baqi—\n\nnot to what perishes.", icon: "arrow.down.heart") }
                    else if currentScreen == 8 { StoryTextBox(text: "Al-Baqi remains—\n\neverything else\nwill vanish.", icon: "wind") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showArabic { VStack(spacing: 16) { Text("كُلُّ مَنْ عَلَيْهَا فَانٍۢ وَيَبْقَىٰ وَجْهُ رَبِّكَ ذُو ٱلْجَلَـٰلِ وَٱلْإِكْرَامِ").font(.system(size: 24, weight: .bold, design: .serif)).foregroundColor(.white).multilineTextAlignment(.center).lineSpacing(10).padding(.horizontal, 30); Text("Surah Ar-Rahman 55:26-27").font(.system(size: 14, weight: .light)).foregroundColor(Color.appGold.opacity(0.7)) }.transition(.opacity.combined(with: .move(edge: .top))) }
                    if showTranslation { Text("\"Everyone upon it will perish,\nand there will remain (yabqa)\nthe Face of your Lord,\nOwner of Majesty and Honor\"").font(.system(size: 19, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Al-Baqi?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "infinity.circle", text: "Al-Baqi is everlasting—everything else will perish.")
                            ExplanationPoint(icon: "smoke", text: "Your wealth, home, loved ones—all temporary.")
                            ExplanationPoint(icon: "arrow.down.heart", text: "Attach your heart to Al-Baqi, not to what will vanish.")
                            ExplanationPoint(icon: "sparkles", text: "Only what you do for Al-Baqi will remain after you're gone.")
                            Text("Al-Baqi—\nthe only One\nwho remains.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
