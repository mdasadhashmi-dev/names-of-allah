// AlBatinStoryView.swift
import SwiftUI
import AVFoundation
struct AlBatinStoryView: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(colors: [Color(red: 0.05, green: 0.07, blue: 0.12), Color.black], startPoint: .top, endPoint: .bottom).ignoresSafeArea()
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "You see a person smiling—\n\nbut you don't see their pain.", icon: "figure.stand") }
                    else if currentScreen == 1 { StoryTextBox(text: "You see the ocean—\n\nbut you don't see\nwhat's beneath.", icon: "water.waves") }
                    else if currentScreen == 2 { StoryTextBox(text: "There are realities\nhidden from you.", icon: "eye.slash") }
                    else if currentScreen == 3 { StoryTextBox(text: "But Al-Batin knows them all.", icon: "brain") }
                    else if currentScreen == 4 { StoryTextBox(text: "The Hidden.\n\nThe Unseen.", icon: "circle.dashed") }
                    else if currentScreen == 5 { StoryTextBox(text: "Why did that hardship happen?\n\nAl-Batin knows.", icon: "questionmark.circle") }
                    else if currentScreen == 6 { StoryTextBox(text: "Why was that door closed?\n\nAl-Batin knows.", icon: "door.left.hand.closed") }
                    else if currentScreen == 7 { StoryTextBox(text: "There is wisdom\nhidden from you—\n\nfor your own good.", icon: "lock.fill") }
                    else if currentScreen == 8 { StoryTextBox(text: "Trust Al-Batin\nwith what's hidden\nfrom you.", icon: "heart.circle") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showTranslation { Text("\"He is the First and the Last,\nthe Manifest and the Hidden (Al-Batin)\"").font(.system(size: 19, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Al-Batin?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "lock.fill", text: "Al-Batin hides certain realities from you for your own protection.")
                            ExplanationPoint(icon: "brain.head.profile", text: "He knows the wisdom behind every hidden matter.")
                            ExplanationPoint(icon: "heart.circle", text: "Trust that what's hidden from you is for your good.")
                            ExplanationPoint(icon: "hands.sparkles", text: "Al-Batin sees what you can't—and that's a mercy.")
                            Text("Al-Batin knows\nwhat you don't—\nand that's enough.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
    ...
    private func autoPlayAyahAudio() {
        if let audioPath = Bundle.main.path(forResource: "al-hadid-57-3-batin", ofType: "mp3") {
