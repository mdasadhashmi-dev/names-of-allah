// AlMateenStoryView.swift - Al-Mateen (The Firm)
import SwiftUI
import AVFoundation

struct AlMateenStoryView: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(colors: [Color(red: 0.1, green: 0.1, blue: 0.14), Color.black], startPoint: .top, endPoint: .bottom).ignoresSafeArea()
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "An earthquake struck.\n\nBuildings crumbled.\nCities fell.", icon: "building.2.crop.circle") }
                    else if currentScreen == 1 { StoryTextBox(text: "Everything humans built—\n\ngone in seconds.", icon: "xmark.circle") }
                    else if currentScreen == 2 { StoryTextBox(text: "But the mountains\nstood still.", icon: "mountain.2") }
                    else if currentScreen == 3 { StoryTextBox(text: "The earth shook—\n\nbut Allah's creation remained firm.", icon: "circle.grid.cross") }
                    else if currentScreen == 4 { StoryTextBox(text: "Al-Mateen—\n\nthe Firm,\nthe Unshakable.", icon: "seal.fill") }
                    else if currentScreen == 5 { StoryTextBox(text: "No earthquake\ncan shake Him.\n\nNo storm\ncan move Him.", icon: "tornado") }
                    else if currentScreen == 6 { StoryTextBox(text: "When your life feels unstable—\n\njob, health, relationships—", icon: "figure.fall") }
                    else if currentScreen == 7 { StoryTextBox(text: "Anchor yourself\nto Al-Mateen.", icon: "anchor") }
                    else if currentScreen == 8 { StoryTextBox(text: "Everything else will shake.\n\nBut He will never move.", icon: "infinity.circle") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showArabic { VStack(spacing: 16) { Text("إِنَّ ٱللَّهَ هُوَ ٱلرَّزَّاقُ ذُو ٱلْقُوَّةِ ٱلْمَتِينُ").font(.system(size: 26, weight: .bold, design: .serif)).foregroundColor(.white).multilineTextAlignment(.center).lineSpacing(10).padding(.horizontal, 30); Text("Surah Adh-Dhariyat 51:58").font(.system(size: 14, weight: .light)).foregroundColor(Color.appGold.opacity(0.7)) }.transition(.opacity.combined(with: .move(edge: .top))) }
                    if showTranslation { Text("\"Indeed, it is Allah who is\nthe Provider, the Possessor of Strength,\nthe Firm (Al-Mateen)\"").font(.system(size: 19, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Al-Mateen?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "mountain.2.fill", text: "Al-Mateen is unshakable—His foundation never trembles, never weakens.")
                            ExplanationPoint(icon: "anchor", text: "When everything in your life feels unstable, anchor yourself to Al-Mateen.")
                            ExplanationPoint(icon: "infinity.circle", text: "His firmness is eternal. He never changes, never falters.")
                            ExplanationPoint(icon: "checkmark.seal", text: "Build your life on Al-Mateen—and you will never be shaken.")
                            Text("Stand firm with Al-Mateen—\nthe foundation\nthat never moves.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
