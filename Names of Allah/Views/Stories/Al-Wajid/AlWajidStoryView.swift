// AlWajidStoryView.swift - Al-Wajid (The Finder)
import SwiftUI
import AVFoundation
struct AlWajidStoryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "You were lost.", icon: "location.slash") }
                    else if currentScreen == 1 { StoryTextBox(text: "No direction.\nNo purpose.\nNo path.", icon: "questionmark.circle") }
                    else if currentScreen == 2 { StoryTextBox(text: "You searched everywhere.", icon: "magnifyingglass") }
                    else if currentScreen == 3 { StoryTextBox(text: "But couldn't find yourself.", icon: "figure.stand") }
                    else if currentScreen == 4 { StoryTextBox(text: "Then Al-Wajid—\nThe Finder—\n\nfound you.", icon: "eye.fill") }
                    else if currentScreen == 5 { StoryTextBox(text: "He found you\nwhen you were lost.", icon: "arrow.down.heart") }
                    else if currentScreen == 6 { StoryTextBox(text: "He gave you purpose.", icon: "sparkles") }
                    else if currentScreen == 7 { StoryTextBox(text: "He showed you the way.", icon: "signpost.right.fill") }
                    else if currentScreen == 8 { StoryTextBox(text: "Al-Wajid finds—\n\nwhat is lost,\nwhat is searching.", icon: "infinity.circle") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showArabic { VStack(spacing: 16) { Text("وَوَجَدَكَ ضَآلًّۭا فَهَدَىٰ").font(.system(size: 40, weight: .bold, design: .serif)).foregroundColor(.white).multilineTextAlignment(.center).padding(.horizontal, 30); Text("Surah Ad-Duha 93:7").font(.system(size: 14, weight: .light)).foregroundColor(Color.appGold.opacity(0.7)) }.transition(.opacity.combined(with: .move(edge: .top))) }
                    if showTranslation { Text("\"And He found (wajadaka) you lost\nand guided you\"").font(.system(size: 24, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Al-Wajid?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "eye.fill", text: "Al-Wajid finds you when you're lost—He sees you.")
                            ExplanationPoint(icon: "signpost.right", text: "He doesn't just find you—He guides you.")
                            ExplanationPoint(icon: "sparkles", text: "Al-Wajid gives purpose to the lost.")
                            ExplanationPoint(icon: "heart.circle", text: "Thank Al-Wajid for finding you when you were searching.")
                            Text("Al-Wajid—\nfinding what\nis lost.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
    ...
    private func autoPlayAyahAudio() {
        if let audioPath = Bundle.main.path(forResource: "ad-duha-93-7", ofType: "mp3") {
