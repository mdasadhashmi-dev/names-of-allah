// AlBasitStoryView.swift - Al-Basit (The Extender)
import SwiftUI
import AVFoundation
struct AlBasitStoryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "You had nothing.\n\nNo money.\nNo help.\nNo way out.", icon: "tray") }
                    else if currentScreen == 1 { StoryTextBox(text: "You raised your hands—\n\n\"Ya Allah, help me.\"", icon: "hands.sparkles") }
                    else if currentScreen == 2 { StoryTextBox(text: "And suddenly,\na door opened.", icon: "door.left.hand.open") }
                    else if currentScreen == 3 { StoryTextBox(text: "Someone called\nwith an opportunity.", icon: "phone.arrow.up.right") }
                    else if currentScreen == 4 { StoryTextBox(text: "Money came\nfrom where you didn't expect.", icon: "banknote") }
                    else if currentScreen == 5 { StoryTextBox(text: "A solution appeared\nout of nowhere.", icon: "lightbulb.fill") }
                    else if currentScreen == 6 { StoryTextBox(text: "That was Al-Basit—\n\nThe Extender.", icon: "arrow.down.heart") }
                    else if currentScreen == 7 { StoryTextBox(text: "He extends His provision\nwhen you least expect it.", icon: "hand.raised.fill") }
                    else if currentScreen == 8 { StoryTextBox(text: "Al-Basit's hand\nis always open—\n\neven when yours is empty.", icon: "infinity.circle") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showTranslation { Text("\"Who will lend Allah a goodly loan?\nAnd Allah withholds and grants abundance (yabsutu)\"").font(.system(size: 19, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Al-Basit?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "hand.raised.fill", text: "Al-Basit extends His provision when you have nothing left.")
                            ExplanationPoint(icon: "door.left.hand.open", text: "He opens doors you never knew existed.")
                            ExplanationPoint(icon: "arrow.down.heart", text: "His blessings come from sources you didn't imagine.")
                            ExplanationPoint(icon: "infinity", text: "Al-Basit's generosity has no limits—only turn to Him.")
                            Text("Al-Basit extends—\nwhen you have\nnothing left to give.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
