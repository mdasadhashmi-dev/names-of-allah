// AlMuizzStoryView.swift - Al-Mu'izz (The Bestower of Honor)
import SwiftUI
import AVFoundation
struct AlMuizzStoryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "People chase honor\nfrom other people.", icon: "figure.run") }
                    else if currentScreen == 1 { StoryTextBox(text: "\"Notice me.\nPraise me.\nRespect me.\"", icon: "star.fill") }
                    else if currentScreen == 2 { StoryTextBox(text: "But that honor\nis temporary.", icon: "clock") }
                    else if currentScreen == 3 { StoryTextBox(text: "It fades.\nIt disappoints.\nIt leaves.", icon: "smoke.fill") }
                    else if currentScreen == 4 { StoryTextBox(text: "Real honor\ncomes from Al-Mu'izz—\n\nThe Bestower of Honor.", icon: "crown.fill") }
                    else if currentScreen == 5 { StoryTextBox(text: "When He honors you,\nno one can\nhumiliate you.", icon: "shield.fill") }
                    else if currentScreen == 6 { StoryTextBox(text: "When people honor you\nand Allah doesn't—\n\nit's meaningless.", icon: "hand.thumbsdown") }
                    else if currentScreen == 7 { StoryTextBox(text: "Seek honor\nfrom Al-Mu'izz—\n\nthrough worship, sincerity,\nand submission.", icon: "hands.sparkles") }
                    else if currentScreen == 8 { StoryTextBox(text: "The honor He gives\nis eternal—\n\nin this life\nand the next.", icon: "infinity.circle") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showTranslation { Text("\"Say: O Allah, Owner of Sovereignty,\nYou give sovereignty to whom You will\nand You honor (tu'izzu) whom You will\nand You humble whom You will\"").font(.system(size: 17, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Al-Mu'izz?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "crown.fill", text: "Al-Mu'izz bestows honor that no one can take away.")
                            ExplanationPoint(icon: "shield.fill", text: "When He honors you, no one can humiliate you.")
                            ExplanationPoint(icon: "heart.circle", text: "Seek His honor through worship, not worldly validation.")
                            ExplanationPoint(icon: "infinity", text: "The honor from Al-Mu'izz is eternal—in this life and the next.")
                            Text("Al-Mu'izz honors—\nwith honor\nthat never fades.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
