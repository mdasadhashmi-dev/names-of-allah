// AlKhafidStoryView.swift - Al-Khafid (The Reducer)
import SwiftUI
import AVFoundation
struct AlKhafidStoryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                Group {
                    if currentScreen == 0 { StoryTextBox(text: "A man walked\nwith his head held high.", icon: "figure.walk") }
                    else if currentScreen == 1 { StoryTextBox(text: "\"Look at me—\nmy wealth,\nmy status,\nmy power.\"", icon: "crown.fill") }
                    else if currentScreen == 2 { StoryTextBox(text: "He forgot\nwhere it all came from.", icon: "questionmark.circle") }
                    else if currentScreen == 3 { StoryTextBox(text: "Then one day—\n\neverything crumbled.", icon: "exclamationmark.triangle") }
                    else if currentScreen == 4 { StoryTextBox(text: "The wealth was gone.\nThe status vanished.\nThe power disappeared.", icon: "smoke.fill") }
                    else if currentScreen == 5 { StoryTextBox(text: "He fell to his knees:\n\n\"Ya Allah, forgive me.\"", icon: "figure.mind.and.body") }
                    else if currentScreen == 6 { StoryTextBox(text: "Al-Khafid—\nThe Reducer—\n\nlowered his pride.", icon: "arrow.down.circle") }
                    else if currentScreen == 7 { StoryTextBox(text: "Not to destroy him—\n\nbut to save him.", icon: "heart.circle") }
                    else if currentScreen == 8 { StoryTextBox(text: "Sometimes Allah lowers you\nto bring you closer to Him.", icon: "hands.sparkles") }
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    if showArabic { VStack(spacing: 16) { Text("خَافِضَةٌۭ رَّافِعَةٌ").font(.system(size: 38, weight: .bold, design: .serif)).foregroundColor(.white).multilineTextAlignment(.center).padding(.horizontal, 30); Text("Surah Al-Waqi'ah 56:3").font(.system(size: 14, weight: .light)).foregroundColor(Color.appGold.opacity(0.7)) }.transition(.opacity.combined(with: .move(edge: .top))) }
                    if showTranslation { Text("\"Bringing low (khafidah)\nand raising high (rafi'ah)\"").font(.system(size: 22, weight: .light, design: .serif)).foregroundColor(.white.opacity(0.9)).multilineTextAlignment(.center).lineSpacing(8).italic().padding(.horizontal, 40).transition(.opacity.combined(with: .scale(scale: 0.9))) }
                    Divider().background(Color.white.opacity(0.2)).padding(.horizontal, 40).padding(.vertical, 8)
                    if showExplanation {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("Why Al-Khafid?").font(.system(size: 22, weight: .semibold, design: .serif)).foregroundColor(.appGold).frame(maxWidth: .infinity, alignment: .center)
                            ExplanationPoint(icon: "arrow.down.circle", text: "Al-Khafid lowers those whose pride has blinded them.")
                            ExplanationPoint(icon: "heart.circle", text: "He lowers you to save you, not to destroy you.")
                            ExplanationPoint(icon: "hands.sparkles", text: "The fall that brings you closer to Allah is a blessing.")
                            ExplanationPoint(icon: "checkmark.seal", text: "Humility before Al-Khafid is better than pride before destruction.")
                            Text("Al-Khafid lowers—\nto bring you\ncloser to Him.").font(.system(size: 18, weight: .medium, design: .serif)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineSpacing(6).frame(maxWidth: .infinity).padding(.top, 10)
    ...
    private func autoPlayAyahAudio() {
        if let audioPath = Bundle.main.path(forResource: "al-waqiah-56-3", ofType: "mp3") {
